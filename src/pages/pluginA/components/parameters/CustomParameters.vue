...
<!-- 三、个性化服务 -->
          ...
          <template v-if="sections.custom">
            <tr><td>验收标准<span class="req">*</span></td><td :class="fieldBgClass('acceptanceStandard')"><el-select v-model="form.acceptanceStandard" size="large" style="width:100%"><el-option v-for="v in opts.acceptanceStandard" :key="v" :label="v" :value="v" /></el-select></td><td class="td-src"><SourceSelector field="acceptanceStandard" :context="props.context" /></td><td class="td-view"><button v-if="showGraphicBtn('acceptanceStandard')" class="btn-view graphic" @click="handleViewClick('acceptanceStandard')">图形</button><button v-if="showDocBtn('acceptanceStandard')" class="btn-view doc" @click="handleViewClick('acceptanceStandard')">加工文档</button></td></tr>
            <tr><td>阻抗控制<span class="req">*</span></td><td :class="fieldBgClass('impedanceControl')"><el-select v-model="form.impedanceControl" size="large" style="width:100%"><el-option v-for="v in opts.impedanceControl" :key="v.value" :label="v.label" :value="v.value" /></el-select></td><td class="td-src"><SourceSelector field="impedanceControl" :context="props.context" /></td><td class="td-view"><button v-if="showGraphicBtn('impedanceControl')" class="btn-view graphic" @click="handleViewClick('impedanceControl')">图形</button><button v-if="showDocBtn('impedanceControl')" class="btn-view doc" @click="handleViewClick('impedanceControl')">加工文档</button></td></tr>
            <tr><td>标记要求<span class="req">*</span></td><td :class="fieldBgClass('markingRequirements')"><el-select v-model="form.markingRequirements" size="large" multiple collapse-tags style="width:100%"><el-option v-for="v in opts.markingRequirements" :key="v" :label="v" :value="v" /></el-select></td><td class="td-src"><SourceSelector field="markingRequirements" :context="props.context" /></td><td class="td-view"><button v-if="showGraphicBtn('markingRequirements')" class="btn-view graphic" @click="handleViewClick('markingRequirements')">图形</button><button v-if="showDocBtn('markingRequirements')" class="btn-view doc" @click="handleViewClick('markingRequirements')">加工文档</button></td></tr>
            <tr><td>周期格式<span class="req">*</span></td><td :class="fieldBgClass('periodFormat')"><el-select v-model="form.periodFormat" size="large" style="width:100%"><el-option v-for="v in opts.periodFormat" :key="v" :label="v" :value="v" /></el-select></td><td class="td-src"><SourceSelector field="periodFormat" :context="props.context" /></td><td class="td-view"><button v-if="showGraphicBtn('periodFormat')" class="btn-view graphic" @click="handleViewClick('periodFormat')">图形</button><button v-if="showDocBtn('periodFormat')" class="btn-view doc" @click="handleViewClick('periodFormat')">加工文档</button></td></tr>
            <tr><td>测试要求<span class="req">*</span></td><td :class="fieldBgClass('testRequirements')"><el-select v-model="form.testRequirements" size="large" multiple collapse-tags style="width:100%"><el-option v-for="v in opts.testRequirements" :key="v" :label="v" :value="v" /></el-select></td><td class="td-src"><SourceSelector field="testRequirements" :context="props.context" /></td><td class="td-view"><button v-if="showGraphicBtn('testRequirements')" class="btn-view graphic" @click="handleViewClick('testRequirements')">图形</button><button v-if="showDocBtn('testRequirements')" class="btn-view doc" @click="handleViewClick('testRequirements')">加工文档</button></td></tr>
            ...
            <tr><td colspan="4" class="remark-cell"><div class="remark-title"><span>📝 备注</span><SourceSelector field="remark" :context="props.context" /></div><div ref="remarkBoxRef" class="remark-box"><template v-if="form.remark.length"><div v-for="(msg, i) in form.remark" :key="i" class="remark-item">{{ String(Number(i) + 1) }}. {{ (msg as string).includes('|') ? (msg as string).split('|').slice(1).join('|') : msg }}</div></template><template v-else><span class="remark-empty">暂无备注信息</span></template></div></td></tr>
          </template>
</template>
