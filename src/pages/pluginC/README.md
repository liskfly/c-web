...
构建产物根目录的 `plugin-config.js` 中，`pluginCRemarkVisible: true` 显示 C 页面备注栏中的 P10 提示，改成 `false` 后只隐藏 P10 提示。接口返回的 `remark` 始终接收、显示和回传，且回传时不会混入页面生成的 P10 提示。修改配置后刷新或重新打开页面即可，不需要重新打包。
